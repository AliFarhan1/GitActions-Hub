...
"""
    ...
    // Fix 3: Recursively collect ALL files including inside folders
        ...
        for file in files {
            ...
            if file.isDirectory {
                ...
                if let children = file.children {
                    ...
                    let childFiles = buildFileTree(at: URL(fileURLWithPath: file.path))
                    let subFiles = collectAllFiles(from: childFiles, basePath: relativePath)
                    result.append(contentsOf: subFiles)
                }
            } else {
    ...
    func createFile(name: String, at parentPath: String, isDirectory: Bool = false) {
        ...
        do {
            if isDirectory {
                ...
                // Fix 3: Add .gitkeep so empty folders can be pushed
...
class GitOperationsManager: ObservableObject {
    ...
    func commitAndPush(owner: String, repo: String, branch: String, message: String, files: [(path: String, content: String)]) async -> Bool {
        ...
        guard !files.isEmpty else {
            await MainActor.run {
                self.lastCommitResult = "❌ لا توجد ملفات نصية للرفع"
        ...
        do {
            ...
            await MainActor.run {
                self.lastCommitResult = "❌ فشل: \(error.localizedDescription)"
                self.isLoading = false
            }
            return false
        }
