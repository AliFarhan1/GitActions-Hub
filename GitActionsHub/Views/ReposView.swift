                        ...
                        ScrollView {
                            LazyVStack(spacing: 10) {
                                ForEach(filteredRepos) { repo in
                                    RepoCard(repo: repo) { selectedRepo = repo; showRepoDetail = true }
                                        .padding(.horizontal)
                                        .onLongPressGesture { repoToDelete = repo; showDeleteAlert = true }
                                }
                            }.padding(.vertical)
                        }
        ...
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let user = gitHubService.currentUser {
                    Text("@\(user.login)").font(.system(size: 13, design: .monospaced)).foregroundColor(AppColors.textSecondary)
                ...
                HStack(spacing: 8) {
                    ...
                    if repo.isPrivate {
                        Text("Private").font(.system(size: 9, weight: .semibold))
                            .foregroundColor(Color(hex: "#FFD93D")).padding(.horizontal, 5).padding(.vertical, 2)
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            HStack {
                                Text(repo.cloneUrl).font(.system(size: 12, design: .monospaced)).foregroundColor(AppColors.text).lineLimit(1)
                                    .environment(\.layoutDirection, .leftToRight)
                                Spacer()
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            Label("كيفية Clone على iPhone", systemImage: "iphone").font(.system(size: 14, weight: .semibold)).foregroundColor(AppColors.text)
                            StepInstructions(steps: [
                                "انسخ Clone URL أعلاه",
                                "افتح تطبيق Files على iPhone",
                                "ادخل: On My iPhone > GitActionsHub",
                                "ضع ملفاتك هنا",
                ...
                VStack(spacing: 0) {
                    ...
                    ScrollView {
                        VStack(spacing: 20) {
                            ...
                            // Fix 3: Developer = علي فرحان
                            VStack(alignment: .leading, spacing: 16) {
                                Label("حول التطبيق", systemImage: "info.circle.fill").font(.system(size: 16, weight: .bold)).foregroundColor(AppColors.text)
                                AppInfoRow(key: "الإصدار", value: "1.0.0")
                                AppInfoRow(key: "التقنية", value: "SwiftUI + GitHub API")
                                AppInfoRow(key: "المطور", value: "علي فرحان")
                                AppInfoRow(key: "GitHub", value: "@AliFarhan1")
                            }
                            .padding(16).glassCard().padding(.horizontal)
                            
                            VStack(alignment: .leading, spacing: 12) {
                                Label("الميزات", systemImage: "star.fill").font(.system(size: 16, weight: .bold)).foregroundColor(AppColors.text)
